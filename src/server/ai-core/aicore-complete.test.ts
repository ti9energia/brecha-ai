...
describe("AI Core completo — RAG por tenant, memória, connector sync (0A)", () => {
  it("ingestDocument isola por tenant: só o tenant que ingeriu recupera", () => {
    ...
    const hitX = inMemoryKnowledge.retrieve("crédito de PIS energia", "org-x", 5);
    ...
    const hitY = inMemoryKnowledge.retrieve("crédito de PIS energia", "org-y", 5);
    expect(hitY.some((c) => c.ref === "Solução de Consulta 99")).toBe(false);
    expect(ingestedCount("org-x")).toBeGreaterThan(0);
  });

  it("memória por usuário lembra e recupera os últimos turnos", () => {
    remember("u-x", { role: "user", content: "quais janelas abrem?" });
    remember("u-x", { role: "assistant", content: "você tem 6 janelas abertas" });
    const turns = recall("u-x");
    expect(turns.length).toBe(2);
    expect(turns[0].role).toBe("user");
    expect(memorySize("u-x")).toBe(2);
    expect(recall("u-inexistente").length).toBe(0);
  });
