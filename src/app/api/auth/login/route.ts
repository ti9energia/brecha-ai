...
export async function POST(req: Request) {
  ...
  const limited = await rateLimit(req, "login", { max: 10, windowMs: 60_000 });
  ...
  const limitedByEmail = await rateLimitBy(email.trim().toLowerCase(), "login-email", { max: 5, windowMs: 60_000 });
