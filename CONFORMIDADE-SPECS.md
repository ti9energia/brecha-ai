...
> Estado do build nesta entrega: **typecheck limpo · 113 testes · build OK**.
>
...
| **0B — WhatsApp** | webhook fail-closed + HMAC; `/send`; push proativo; auditoria; mesmo cérebro; **"responda SIM" ✅**; **opt-in por código ✅**; **4 línguas ✅** | — | mídia (áudio/foto/PDF), gating por plano | **Canal ✅, governança ✅** |
...
4. 🟡 **PARCIAL — WhatsApp: confirmação "responda SIM" e opt-in por código FECHADOS.** Confirmação de ação sensível (só executa após SIM, auditado) e **opt-in com verificação por código** (`requestWhatsappOptIn`/`confirmWhatsappOptIn` + `POST /api/whatsapp/optin`; o número responde o código pelo WhatsApp e fica vinculado dinamicamente) — ambos testados (`gateway.ts`). **Ainda faltam:** gating por plano do canal e entrada de mídia (áudio/foto/PDF). *(0B §3 DoD a ✅, §8 DoD c ✅ · §4 mídia / §8 DoD h ❌)*
...
7. **WhatsApp restante** (#4): ✅ opt-in por código feito; faltam entrada de mídia e gating por plano.
